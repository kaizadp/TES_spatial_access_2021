...
```{r}
data_long = read.csv("data/processed/fticr_long_core.csv.gz")
...
data_long_trt = read.csv("data/processed/fticr_long_trt.csv.gz")
...
```{r}
relabund_cores = 
   read.csv("data/processed/fticr_relabund_cores.csv") %>% 
   dplyr::mutate(class = factor(class, levels = 
                          c("amino_sugar", "carbohydrate","lipid","protein","unsat_hc",
                            "condensed_hc","lignin","tannin", "other")),
                 Amendments = factor(Amendments, levels = c("control", "C", "N")),
...
relabund_wide = 
  relabund_cores %>% 
  dplyr::select(Core, Assignment, class, relabund, Moisture, Wetting, Suction, Homogenization, Amendments) %>% 
  spread(class, relabund) %>% 
  replace(is.na(.),0)

# create a matrix of all the group columns
relabund_wide$DV = as.matrix(relabund_wide[,8:16])
...
relabund_pca=
  relabund_cores %>% 
  select(Core, Assignment, class, relabund, Moisture, Wetting, Suction, Homogenization, Amendments) %>% 
  spread(class, relabund) %>% 
  replace(is.na(.),0) %>% 
  select(-1,)
...
The PCA of FTICR in intact cores shows that the control cores (no amendment) showed greater influence of  
